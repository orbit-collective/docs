...
- **[Laravel 13](https://laravel.com)** (PHP 8.5+) — routing, Eloquent ORM, validation, sessions
...
- Session, cache, and queue all default to the `database` driver — the queue is no longer optional infrastructure: a worker process must be running for [email notifications](/concepts/notifications#channels) to actually be delivered
- **[Pest](https://pestphp.com)** for backend tests, run against an in-memory SQLite database
- **[nsfwjs](https://github.com/infinitered/nsfwjs)**, run as a separate HTTP service, screens every uploaded [avatar](/concepts/account#profile) before it's accepted
...
- **Tailwind CSS**, driven by CSS custom properties (`--accent-color`, `--bg-color`, and so on) consumed as Tailwind arbitrary values — supports Light, Dark, and System theme modes plus a configurable accent color, see [Appearance](/guides/appearance)
