...
- **`ActivityLogService`** — appends a human-readable sentence to the activity log whenever something changes. Most calls pass a project (an issue is created, updated, or deleted; a comment is added; project columns are updated) alongside the acting user. Every mutation under [Settings → Account](/concepts/settings) instead passes `null` for the project — renaming your profile, changing your avatar or password, revoking a session, updating session lifetime, deleting your account, and changing notification settings all log this way, since none of them belong to any one project. See [Activity log](/concepts/activity-log) for the full split.
