...
<ResponseField name="Notification" type="concept">
  A per-user alert about something relevant to you (assignment, a comment, a status/priority/label/date change), delivered on one or both of two channels: in-app and email. Not to be confused with the activity log. See [Notifications](/concepts/notifications).
</ResponseField>
...
<ResponseField name="Settings" type="concept">
  The dedicated `/settings` page, split into an Account section (Preferences, Profile, Notifications, Security & access) and a Workspace section. Most Workspace tabs are placeholders today. See [Settings](/concepts/settings).
</ResponseField>
